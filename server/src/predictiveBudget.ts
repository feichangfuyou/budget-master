...
import type { Transaction, BudgetForecast } from './types.js';
...
export class PredictiveBudgetEngine {
  ...
  private prepareSequences(
    data: Array<{ date: Date; amount: number }>,
    lookback: number
  ): { inputs: tf.Tensor3D; targets: tf.Tensor2D } {
  ...
  > {
    ...
    for (let i = 0; i < daysAhead; i++) {
      ...
      const predValue = (await pred.data())[0] as number;
      input.dispose();
      pred.dispose();
  ...
  private generateRecommendations(
    category: string,
    predictions: Array<{ date: Date; predicted: number }>,
    ...
    if (trend.direction === 'increasing' && trend.strength > 0.5) {
      recs.push(
        `📈 ${category} spending is trending up (+${(trend.strength * 100).toFixed(0)}% confidence). Expected ${(avgDaily * 30).toFixed(0)}/month. Consider setting a budget cap.`
      );
    } else if (trend.direction === 'decreasing' && trend.strength > 0.5) {
      recs.push(
        `📉 Great job! ${category} spending is trending down. You're on track to save $${(avgDaily * 30).toFixed(0)}/month.`
      );
    }
    if (seasonality) {
  ...
  private insufficientDataForecast(category: string): BudgetForecast {
    return {
      category,
      predictions: [],
      trend: 'stable',
  ...
  async exportModel(category: string): Promise<void> {
