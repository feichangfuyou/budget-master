...
exports.IntelligentRouter = exports.MODELS = void 0;
exports.MODELS = {
    local: {
        tier: 'local',
        name: 'browser-local',
...
class IntelligentRouter {
    route(query, context) {
        if (this.isSimpleCategorization(query))
            return 'local';
    ...
    isSimpleCategorization(query) {
        return SIMPLE_CATEGORIZATION_PATTERNS.some((p) => p.test(query));
    }
    assessComplexity(query, context) {
        let score = 0;
        if (/and then|based on/.test(query))
            score += 3;
        const count = context.relevant?.length ?? context.transactions?.length ?? 0;
        if (count > 50)
            score += 2;
        if (/trend|forecast|predict|pattern/i.test(query))
            score += 3;
        if (/compare|versus|vs\.?|difference/i.test(query))
            score += 2;
        if (/calculate|sum|total|average|percentage/i.test(query))
            score += 1;
        if (/suggest|recommend|advice|ideas/i.test(query))
            score += 4;
        return score;
    }
}
