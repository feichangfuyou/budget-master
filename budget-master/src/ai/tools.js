...
exports.BUDGET_TOOLS = void 0;
exports.createDefaultToolHandlers = createDefaultToolHandlers;
exports.BUDGET_TOOLS = [
    {
...
function createDefaultToolHandlers(getTransactions) {
    ...
    return {
        async query_transactions(params) {
            ...
            if (params.minAmount != null)
                list = list.filter((t) => t.amount >= params.minAmount);
            if (params.maxAmount != null)
                list = list.filter((t) => t.amount <= params.maxAmount);
            if (params.startDate)
                list = list.filter((t) => new Date(t.date).toISOString().slice(0, 10) >= params.startDate);
            if (params.endDate)
                list = list.filter((t) => new Date(t.date).toISOString().slice(0, 10) <= params.endDate);
            if (params.merchant)
                list = list.filter((t) => (t.merchant ?? '').toLowerCase().includes(params.merchant.toLowerCase()));
