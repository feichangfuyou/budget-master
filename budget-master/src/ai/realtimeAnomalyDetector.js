...
function toResult(isAnomaly, score, type, explanation, recommendation) {
    if (!isAnomaly)
        return null;
...
class RealtimeAnomalyDetector {
    ...
    detectAmountAnomaly(transaction, history) {
        ...
        return toResult(isAnomaly, score, 'amount', `${transaction.category ?? 'Uncategorized'} purchase of $${transaction.amount.toFixed(2)} is ${zScore.toFixed(1)}σ from your average of $${avg.toFixed(2)}`, isAnomaly ? `This is ${((transaction.amount / avg - 1) * 100).toFixed(0)}% higher than normal. Verify this transaction.` : undefined);
    }
    detectFrequencyAnomaly(transaction, history) {
        ...
        const isAnomaly = score > ALERT_THRESHOLDS.low;
        return toResult(isAnomaly, score, 'frequency', `Usually ${avgInterval.toFixed(0)} days between purchases at ${merchant}, been ${lastTransactionDays.toFixed(0)} days`, isAnomaly ? `Unusual timing for ${merchant}. You typically shop here every ${avgInterval.toFixed(0)} days.` : undefined);
    }
    detectNewMerchant(transaction, history) {
        ...
        return toResult(isAnomaly, score, 'merchant', seen ? `Known merchant: ${merchant}` : `First transaction with ${merchant}`, !seen ? 'New merchant detected. Verify this is a legitimate charge.' : undefined);
    }
    detectCategoryShift(transaction, history) {
        ...
        const isAnomaly = score > ALERT_THRESHOLDS.low;
        return toResult(isAnomaly, score, 'category', `${transaction.category} represents ${(currentCategoryFreq * 100).toFixed(1)}% of recent spending`, isAnomaly ? `Unusual ${transaction.category} purchase. This category is typically ${(currentCategoryFreq * 100).toFixed(1)}% of your spending.` : undefined);
    }
    detectTimeAnomaly(transaction, history) {
        ...
        const isAnomaly = score > ALERT_THRESHOLDS.low;
        return toResult(isAnomaly, score, 'time', `Transaction at ${hour}:00 (${(hourFreq * 100).toFixed(1)}% of your usual activity)`, isAnomaly ? `Unusual transaction time. You rarely make purchases at ${hour}:00.` : undefined);
    }
    calculateMAD(values) {
...
//# sourceMappingURL=realtimeAnomalyDetector.js.map